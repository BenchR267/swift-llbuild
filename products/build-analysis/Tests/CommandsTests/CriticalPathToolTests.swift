...
class CriticalPathToolTests: XCTestCase {
    ...
    func createSimplePath() throws -> CriticalPath<PathElement> {
        ...
        return try CriticalPath([PathElement(key: key, result: result, dependencies: [])])
    }
    
    func testJsonEncoding() throws {
        let tool = try CriticalPathTool(args: ["-d", ""])
        
        XCTAssertEqual(try tool.json(.empty()).string, #"{"error":"No path calculated."}"#)
